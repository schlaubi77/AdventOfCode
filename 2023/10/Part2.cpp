...
int main() {
    ...
    int pointsWithinQuarter = 0; // https://www.reddit.com/r/adventofcode/comments/18ez5jb/2023_day_10_part_2_shortcut_solution_using_shape/
    while(grid[y][x] != 'S') {
        pair<int, int> pos = findNext(grid, y, x);
        y = pos.first;
        x = pos.second;
        if(y >= grid.size() / 4 && y < 3 * grid.size() / 4 && x >= grid[0].length() / 4 && x < 3 * grid[0].length() / 4) {
            pointsWithinQuarter++;
        }
    }
    cout << grid.size() * grid[0].length() / 4 - pointsWithinQuarter << endl;
}