...
int testFrom(vector<string>& grid, vector<vector<bool>> energizationMap, int y, int x, char direction) {
    beamStartsHandled.clear();
    moveBeam(grid, energizationMap, y, x, direction);
...
int main() {
    ...
    for(int y = 0; y < grid.size(); y++) {
        maxEnergized = max(maxEnergized, testFrom(grid, energizationMap, y, 0, 'R'));
        maxEnergized = max(maxEnergized, testFrom(grid, energizationMap, y, grid[0].length() - 1, 'L'));
    }
    for(int x = 0; x < grid[0].length(); x++) {
        maxEnergized = max(maxEnergized, testFrom(grid, energizationMap, 0, x, 'D'));
        maxEnergized = max(maxEnergized, testFrom(grid, energizationMap, grid.size() - 1, x, 'U'));
    }
