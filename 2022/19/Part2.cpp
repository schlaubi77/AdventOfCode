...
int getBestGeodes(Blueprint bp, State state) {
    ...
    for (int i = state.timePassed; i < 32; i++) {
...
int main()
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        result *= getBestGeodes(blueprints[i], baseState);
    }
    ...
    // could probably be sped up by saveing the function's results
    return 0;
}